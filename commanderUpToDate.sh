...
# This script checks (or records) the HEAD githash of the commander-cli repo (branch 'master')
# and can be used to detect whether it was changed since it was last recorded
# Syntax:
#   ./commanderUpToDate
# or
#   ./commanderUpToDate record
...
if [ ! -d ../commander-cli ]; then
    echo "commander-cli is not a peer of nimbella-cli or nimbella-cli is not the current directory"
    exit 0
fi
pushd ../commander-cli > /dev/null
