...
Thes triggers are examined in this order, and when the deployer finds one, it does not consider the other. The exception is that a script in `build.sh` or `build.cmd` can always do its own `npm install` or `yarn install`.
