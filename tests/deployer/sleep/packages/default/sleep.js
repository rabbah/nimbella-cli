...
function main(parm) {
    if(!('sleepTimeInMs' in parm)) {
        const result = { body: { error: "Parameter 'sleepTimeInMs' not specified." }}
        console.error(result.error)
        return result
    }

    const sleepTime = new Number(parm.sleepTimeInMs)

    console.log("Specified sleep time is " + sleepTime + " ms.")

    return new Promise(function(resolve, reject) {
        const timeBeforeSleep = new Date()
        setTimeout(function () {
            const actualSleepTimeInMs = new Date() - timeBeforeSleep
            const result = { body: {msg: "Terminated successfully after around " + actualSleepTimeInMs + " ms." }}
